...
const SceneContent = ({ params, isGodView = false }: { params: SimulationParams, isGodView?: boolean }) => {
  ...
  useFrame((state, delta) => {
    if (meshRef.current && !params.isPaused) {
      ...
      // Deterministic Floating (Bobbing)
      // Using rotation.y as a time proxy ensures that position is tied to the deterministic rotation state
      // This avoids random seed issues from <Float> component
      if (!isGodView) {
         // Vertical bobbing
...
const CameraVisualizer = ({ params }: { params: SimulationParams }) => {
  ...
  // Approximate unscaled dimensions of the camera parts for bounding box
  // Box: 0.4 x 0.3 x 0.6 (centered 0,0,0)
  // Cone: d=0.5, h=0.4 (z: -0.3 to -0.7)
  // Lens: r=0.15 (z: -0.7 to -0.85)
  // Total Z range: +0.3 to -0.85 = 1.15
  // Max width/height: 0.5 (Cone base)
  const camDims: [number, number, number] = [0.5, 0.5, 1.15];
  // The center of geometry is shifted along Z. 
  // Center Z = (0.3 + -0.85) / 2 = -0.275
  ...
  const CameraMesh = ({ color, pos, rot, label }: { color: string, pos: [number, number, number], rot: [number, number, number], label: string }) => (
    <group position={pos} rotation={rot}>
      ...
      <group scale={[scale, scale, scale]}>
        ...
        {/* Camera Bounding Box (Scaled with camera) */}
        {params.showCameraBoundingBox && (
          <group position={camCenterOffset}>
            <BoundingBoxOverlay 
              dims={camDims} 
              scale={scale} // Pass scale for correct label calculation
              color={color} 
              showLabels={true} 
      ...
      {/* Main Label (L/R) - Positioned above the visual elements */}
      {/* 
        Adjusted Height:
        Camera Top (approx) = 0.25 * scale (Cone radius)
        Text should sit just above that. 
        Font Size adjusted to be proportional to camera width (0.4 * scale)
      */}
      <Text 
        position={[0, (0.35 * scale), 0]} // Position just above the cone/box
        fontSize={0.35 * scale} // Font size proportional to camera body width (~0.4)
        color="white" 
        anchorX="center" 
        anchorY="bottom"
...
export const SimulationCanvas: React.FC<SimulationCanvasProps> = ({ params, viewMode }) => {
  ...
  // NEW: Calculate Vertical FOV based on Focal Length
  // Assuming standard Full Frame sensor (height = 24mm)
  // Formula: FOV = 2 * atan(SensorHeight / (2 * FocalLength))
  const fov = useMemo(() => {
